...
API happy-case 통합 테스트 충분성, edge-case 선택, 구현 세부 결합 여부는 script가 자동 판정하지 않고 `state-trail-test-writing` 및 `state-trail-code-review` guidance와 reviewer 판단으로 남깁니다. 현재 sample LogQL definition은 `live: optional`이므로 strict harness는 live query가 설정되지 않았음을 보고하고 통과할 수 있습니다. `live: required` definition이 추가되면 strict harness는 Loki URL, `logcli`, 필요한 인증 환경변수가 없을 때 실패해야 합니다.
